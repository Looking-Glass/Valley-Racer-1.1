	...
	public void SetupMountains() {
		...
		for (var y = 0; y < h; y++) {
			for (var x = 0; x < w1; x++) {
				...
				if (y > 0) { //don't draw peaks at y = 0. it is just annoying. invisible peaks show up.
					float peakColor;
					if (x != w) {
					...
					bool randomPeak = false;
					if (Random.value < percentRandom && y != 0 && y != h - 1) {
						randomPeak = true;
					}

					if (peakColor > 0 || randomPeak) {
						if (x != 0) { //so that there's only one peak per peak
							GameObject newPeak = Instantiate (peak);
							newPeak.transform.position = verts [(y * w1) + x];
							newPeak.transform.parent = this.transform;
							if (randomPeak) {
		...
		for (var y = 0; y < h - 1; y++) {
			for (var x = 0; x < w1 - 1; x++) {
				if (peaks[((y + 1) * w1) + x] == true || peaks[(y * w1) + x + 1] == true) {
		...
		for (var i = 0; i < verts.Length; i++) {
			...
			if (i != 0 && (i+1) % w1 == 0) { //this matches the verts along x = 0 to the ones at x = full width.
		...
		{//experiment with flat faces inside the mtn
			...
			volumeChild.transform.parent = this.transform;
